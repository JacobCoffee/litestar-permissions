...
install-prek: ## Install prek and install hooks (ALWAYS use prek, NEVER pre-commit directly)
	@echo "=> Installing prek hooks (pre-commit wrapper with better UX)"
	@$(UV) run prek install
...
lint: ## Runs prek hooks (ruff check, ruff format, ty, codespell, sphinx-lint)
...
test-cov: ## Run tests with coverage report
	@PYTHONDONTWRITEBYTECODE=1 $(UV) run --no-sync pytest --cov=src/litestar_permissions --cov-report=html --cov-report=term
