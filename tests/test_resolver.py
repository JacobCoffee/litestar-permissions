...
def test_resolver_global_permission(db_session, rbac_models):
    ...
    assert resolver.can(user_id, "application:deploy", db=db_session) is True
    assert resolver.can(user_id, "nonexistent:perm", db=db_session) is False


def test_resolver_scoped_permission(db_session, rbac_models):
    ...
    assert resolver.can(user_id, "project:create", "organization", org_id, db=db_session) is True
    # Does NOT have permission for a different org
    assert resolver.can(user_id, "project:create", "organization", uuid4(), db=db_session) is False


def test_resolver_caching(db_session, rbac_models):
    ...
    assert resolver.can(user_id, "app:view", db=db_session) is True
